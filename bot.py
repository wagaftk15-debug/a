...
def set_cache(key, data):
    """احفظ بيانات في cache فقط إذا كانت صحيحة"""
...
def send_invoice(chat_id, amount, title, description, payload_str):
    """إرسال فاتورة (تم التصحيح لعملة XTR نجوم تيليجرام)"""
    if not chat_id or amount <= 0:
        return False
    try:
        # بالنسبة لنجوم تيليجرام يجب أن يكون الـ provider_token فارغاً تماماً
        payload = {
            "chat_id": int(chat_id),
            "title": title[:32],
        ...
        if not success:
            ...
            send_message(chat_id, f"⚠️ خطأ في إنشاء الفاتورة، الرجاء المحاولة لاحقاً.")
        else:
...
def webhook():
    ...
    if "message" in update:
        ...
        if "successful_payment" in msg:
            ...
            if invoice_payload and invoice_payload.startswith("suggest_"):
                send_message(chat_id, "✅ شكراً على الدعم! الآن اكتب اقتراحك:")
                ...
                if success:
                    ...
                    if ADMIN_CHAT_ID:
                        send_message(ADMIN_CHAT_ID, f"✅ دعم جديد: {user_id} × {amount}⭐")
                else:
                    send_message(chat_id, "⚠️ تم الدفع لكن حدث خطأ في التسجيل")
            return jsonify({"ok": True})

        if text.startswith("/start"):
            send_message(chat_id, "👋 أهلاً فيك في <b>بوت زوجوني</b> 💍\n\nاختر ما يناسبك:", main_keyboard())
...
<body>
    <div class="container">
        ...
        <div class="tabs">
            ...
            <button class="tab-btn" onclick="switchTab('donate')">الدعم</button>
        </div>

        <div id="home" class="tab-content active">
            <p style="text-align:center;">أهلاً بك في واجهة الويب المصغرة للبوت.</p>
        </div>

        <div id="donate" class="tab-content">
            <p style="text-align:center;">يمكنك دعمنا من خلال البوت باستخدام نجوم تيليجرام ⭐️</p>
        </div>
    </div>

    <script>
        // إعداد تطبيق ويب تيليجرام
...
if __name__ == "__main__":
    init_db()
    # يتم تشغيل التطبيق على المنفذ المحدد في متغيرات البيئة (مناسب للاستضافة)
